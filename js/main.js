...
(function() {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const scrollTopButton = document.querySelector("#scroll-top"); // 스크롤 버튼 선택
    const mainBanner = document.querySelector("#MainBanner"); // 메인 배너 섹션 선택
  
    // 스크롤 이벤트에 따른 표시 여부 토글 함수
    function toggleScrollTopVisibility() {
      const mainBannerBottom = mainBanner.getBoundingClientRect().bottom; // 메인 배너 섹션의 하단 위치
  
      if (mainBannerBottom < 0) {
        // 메인 배너가 화면에서 벗어나면 버튼 표시
